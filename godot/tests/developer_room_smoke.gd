...
func _run() -> void:
	...
	_expect(is_equal_approx(DeveloperResonanceRenderer._lissajous_clock(lissajous_period * 2.0, lissajous_group) - DeveloperResonanceRenderer._lissajous_clock(0.0, lissajous_group), TAU), "Lissajous projection parameter completes one cycle per two cascade periods")
	...
	var mst_candidates: Array[Dictionary] = [
		...
		{"first": Vector2.RIGHT, "second": Vector2.ONE},
		{"first": Vector2.ONE, "second": Vector2.DOWN},
		{"first": Vector2.DOWN, "second": Vector2.ZERO},
